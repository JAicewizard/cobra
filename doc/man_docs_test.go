...
func TestGenManDoc(t *testing.T) {
	...
	if err := GenMan(echoCmd.DocumentationData(), header, buf); err != nil {
		t.Fatal(err)
	}
	output := buf.String()
...
func TestGenManSeeAlso(t *testing.T) {
	...
	buf := new(bytes.Buffer)
	header := &GenManHeader{}
	if err := GenMan(rootCmd.DocumentationData(), header, buf); err != nil {
		t.Fatal(err)
	}
...
func TestGenManTree(t *testing.T) {
	...
	if err := GenManTree(c.DocumentationData(), header, tmpdir); err != nil {
...
func BenchmarkGenManToFile(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		if err := GenMan(rootCmd.DocumentationData(), nil, file); err != nil {
			b.Fatal(err)
		}
	}
}
